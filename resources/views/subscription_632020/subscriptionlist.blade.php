									  ...
									  <table id="subscriptionmemberlist_datatable" class="table">
										...
										<tbody>
											...
											<tr>
												...
												<td>
													@php
														$cmp_detail = url("/subscriptioncompanylist/{$row->company_id}");
														$company_name = DB::table('companies')->where('id' ,$row->company_id)->value('company_name');
														echo '<a href="'.$cmp_detail.'">'.$company_name.'</a>';
													@endphp
												</td>
												<td style="text-align:center">{{ $row->total }}</td>
												<td style="text-align:center">
													<form method="POST" action="{{ route('subscriptionCompanyDelete',$row->company_id) }}">
														{{ csrf_field() }}
														{{ method_field('DELETE') }}
...
<script>
	$(document).ready(function() {
		$('#subscriptionmemberlist_datatable').DataTable({
			responsive: true,
			lengthMenu: [
				[10, 25, 50, 100],
				[10, 25, 50, 100]
